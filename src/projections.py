...
URL = 'https://fantasy.espn.com/apis/v3/games/ffl/seasons/2019/segments/0/leagues/193035?scoringPeriodId=0&view=kona_player_info'
cookies = {
...
def persist(projections):
    ...
    player_projections = map(lambda x: (x.get('fullName'), x.get('proTeamId'), positions.get(x.get('defaultPositionId')), x.get('stats')[1].get('appliedTotal')), players)
